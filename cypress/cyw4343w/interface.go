...
func (c *Cyw4343w[SDIO]) initBackplane() error {
	...
	err = c.writeRegisterValue(busFunction, sdiodCccrF1blksize0, 1, 64)
	if err != nil {
		return err
	}

	err = c.writeRegisterValue(busFunction, sdiodCccrF2blksize0, 1, 64)
	if err != nil {
		return err
	}
...
func (c *Cyw4343w[SDIO]) downloadFirmware() error {
	...
	firmware := unsafe.Slice(unsafe.StringData(firmware), len(firmware))
	nvram := unsafe.Slice(unsafe.StringData(nvram), len(nvram))
...
func (c *Cyw4343w[SDIO]) writeBackplaneValue(address uint32, length uint8, value uint32) error {
	...
	err = c.write(data[:min(4, length)], sdio.Transfer{
		Address:    address,
		BlockSize:  64,
		BlockCount: 1,
		Function:   uint8(backplaneFunction),
		Increment:  true,
	})
	if err != nil {
		return err
...
func (c *Cyw4343w[SDIO]) readBackplaneValue(address uint32, length uint8) (uint32, error) {
	...
	var data [4]byte
	err = c.read(data[:min(4, length)], sdio.Transfer{
		Address:    address,
		BlockSize:  64,
		BlockCount: 1,
		Function:   uint8(backplaneFunction),
...
func (c *Cyw4343w[SDIO]) readBackplaneBytes(address uint32, data []byte) error {
	...
	for remaining > 0 {
		...
		err = c.read(readBuf, sdio.Transfer{
			BlockSize: 64,
			Address:   transAddr,
			Function:  uint8(backplaneFunction),
			Increment: true,
...
func (c *Cyw4343w[SDIO]) writeRegisterValue(function busFunctionType, address uint32, length uint8, value uint32) error {
	var data [4]byte
	binary.LittleEndian.PutUint32(data[:], value)
	return c.write(data[:min(4, length)], sdio.Transfer{
		Address:    address,
		BlockSize:  64,
		BlockCount: 1,
...
func (c *Cyw4343w[SDIO]) write(buf []byte, t sdio.Transfer) error {
	// Send the packet.
	var err error
	if len(buf) >= 64 {
		err = c.host.WriteBlocks(buf, t)
	} else {
		err = c.host.WriteBytes(buf, t)
	}
	return err
}

func (c *Cyw4343w[SDIO]) read(buf []byte, t sdio.Transfer) error {
	// Send the packet.
	var err error
	if len(buf) >= 64 {
		err = c.host.ReadBlocks(buf, t)
	} else {
		err = c.host.ReadBytes(buf, t)
	}
	return err
}
